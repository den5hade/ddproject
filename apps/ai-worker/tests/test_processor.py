...
import pytest
from app.ai_client import ExtractionResult
...
def _processor(mock_s3, mock_publisher):
    settings = Settings(
        s3_tenant_id="test-tenant",
        ai_api_key="test-key",
        prompts_dir="app/prompts",
    )
    proc = DocumentProcessor(mock_s3, mock_publisher, settings)
    proc._ai_client.extract_text_from_image = AsyncMock(return_value="# Page text")
    proc._ai_client.extract_canonical = AsyncMock(
...
@pytest.fixture
def processor(mock_s3, mock_publisher):
    return _processor(mock_s3, mock_publisher)


@pytest.mark.asyncio
async def test_converting_publishes_document_converted(processor, mock_s3, mock_publisher):
    event = _event(DocumentUploaded)

    with patch("app.processor.convert_pdf_to_images", return_value=[b"page1"]):
...
async def test_structuring_publishes_document_analysis_completed(
    ...
    await processor.handle_structuring(converted)
...
@pytest.mark.asyncio
async def test_structuring_uploads_valid_canonical_json(processor, mock_s3, mock_publisher):
    event = _event(DocumentUploaded)
    mock_s3.download_bytes.return_value = b"# Some unstructured text"
    converted = DocumentConverted(
        event_id=uuid4(),
...
async def test_converting_failure_publishes_failure(mock_s3, mock_publisher):
    ...
    with patch(
        "app.processor.convert_pdf_to_images",
...
async def test_structuring_frontmatter_includes_source_metadata(mock_s3, mock_publisher):
    ...
    await proc.handle_structuring(converted)
